        ...
        <div className="postCommentCard">
            ...
            <>
                <div className="postCommentCarduser">
                    ...
                    <div className="postCommentCarduserinfo">
                        <img className="comment-image" style={{width: '31px', height: '31px', borderRadius: '50%'}} src={comment.user.avatar || ProfileImg} alt={comment.user.fullname}/>
                        <div className="info">
                        ...
                        <>
            {
              ...
              <Link to ={`/profile/${comment.tag._id}`}>
                ...
                <div className="commentcontent-likes">
                <p className="commentcontent-likescount">{comment.likes.length}</p> <FavoriteBorderIcon style={{color:'red'}}/> 
                
                { onEdit 
                ? 
                    <>
                    <p className="commentcontent-reply" onClick={()=>handleupdatecomment()} style={{cursor:'pointer'}}>update</p>
                    <p className="commentcontent-reply" onClick={()=>setonEdit(false)}  style={{cursor:'pointer'}}>cancel</p>
                    </>
                    :
                    <p className="commentcontent-reply"  style={{cursor:'pointer'}}  onClick={handleReply}>{onReply ? 'replyopinion' : 'Reply' }</p>
                    } 
                 </div>
